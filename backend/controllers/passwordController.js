...
const verifyOtp = async (req, res) => {
  ...
  try {
    ...
    console.log(`Reset token", reset:${email}`);
...
const resetPasswordRequest = async (req, res) => {
  ...
  try {
    ...
    await client.set(`reset:${email}`, resetToken, 'EX', 3600);
    console.log(`Token set for ${email}:`, await client.get(`reset:${email}`));
...
const resetPassword = async (req, res) => {
  ...
  try {
    ...
    // const email = await client.get(`reset:${resetToken}`);
    // if (!email) {
    //   return res.status(400).json({ success: false, error: "Invalid or expired token." });
    // }

    console.log("Checking reset token for email:", email);
